<template>
  <div class="tab-content">
    <el-table :data="tableData" stripe style="width: 100%" @cell-click="cellClick" :cell-class-name="cellClassName">
      <el-table-column v-for="(head,index) in tableHead" v-if="head.type === 'img'" :key="index" :prop="head.prop" :label="head.label" :width="head.width" :show-overflow-tooltip="true">
        <template slot-scope="scope">
                          <img :src="scope.row.headImg" class="userImg">
</template>
                </el-table-column>
...
<template slot-scope="scope">
  <el-button v-for="(tab,index) in tabSet" :key="index" @click="deleteRow(scope.$index, scope.row,index)" type="text" size="small">
    {{tab.lable}}</el-button>
</template>
                </el-table-column>    
</el-table>
    
 <div class="block" style="text-align: center;margin-top:30px" v-show="page.pageContent>1">
